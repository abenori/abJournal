...
public struct HPdfDate {
    ...
    public HPdfDate(System.DateTime dt){
		year = dt.Year;
		month = dt.Month;
		day = dt.Day;
		hour = dt.Hour;
        ...
        seconds = dt.Second;
        switch(dt.Kind) {
        case DateTimeKind.Utc: ind = 'Z'; break;
        case DateTimeKind.Local: ind = '+'; break;
        default: ind = ' '; break;
        }
        var x = dt.ToLocalTime() - dt.ToUniversalTime();
        if(ind == ' ') {
            off_hour = 0;
            off_minutes = 0;
        } else {
            off_hour = x.Hours;
            off_minutes = x.Minutes;
        }
	}
};
